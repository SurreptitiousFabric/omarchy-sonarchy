...
| Browse local library by artist, album artist, album, genre, composer, track, share, and imported playlist | **Implemented for categories reported by the speaker.** Sonarchy provides bounded pages, breadcrumbs, Back navigation, track search, and identity-checked playback. It does not add shares, store SMB credentials, or manage the index schedule. |
