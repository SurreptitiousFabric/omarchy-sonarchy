...
from sonarchy_backend.controller import ControllerError, SonosController
...
def test_refresh_builds_target_and_playback(tmp_path):
    ...
    assert set(snapshot["target"]["memberUids"]) == {"R1", "R2"}
    ...
    assert snapshot["playback"]["artworkKind"] == "track"
    room_states = {
...
def test_radio_uses_media_metadata_when_track_metadata_is_blank(tmp_path):
    ...
    living.avTransport.media_info = {
        ...
        "CurrentURIMetaData": (
            ...
            "<upnp:albumArtURI>https://cdn.radiotime.com/station.png</upnp:albumArtURI>"
            "</item></DIDL-Lite>"
        ),
    }
...
def test_complete_media_title_replaces_truncated_track_title(tmp_path):
    ...
    assert snapshot["playback"]["title"] == ("Luigi Mangione’s High-Risk Legal Strategy")


def test_speaker_metadata_rejects_xml_entities():
    malicious = (
        '<!DOCTYPE item [<!ENTITY payload "expanded">]>'
        '<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/">'
        "<item><dc:title>&payload;</dc:title></item></DIDL-Lite>"
    )

    result = SonosController._media_metadata(
        {"CurrentURI": "x-test:stream", "CurrentURIMetaData": malicious}
    )

    assert result == {"title": "", "artworkUrl": "", "uri": "x-test:stream"}
...
def test_group_volume_and_seek(tmp_path):
    controller, living, _, group = make_controller(tmp_path)
    controller.refresh()
...
def test_favorite_artwork_is_safely_made_speaker_local(tmp_path):
    controller, living, _, _ = make_controller(tmp_path)
    living.music_library = FakeMusicLibrary(
        [
            FakeFavorite(
                "Station",
                "x-sonosapi-stream:123",
                album_art_uri="/getaa?s=1&u=station",
            )
        ]
    )
    ...
    assert snapshot["favorites"]["items"][0]["albumArtUrl"] == (
        "http://10.0.0.2:1400/getaa?s=1&u=station"
    )
...
def test_tunein_podcast_favorite_plays_latest_episode_without_developer_account(
    ...
    reference = FakeReference(
        item_id=("100b2064p295446%3Atopic--2e8792e144a1455a82a2c35380306c07"),
                ...
                "track_metadata": type(
                    ...
                    (),
                    {
                        "metadata": {
                            ...
                            "host": "Josh and Chuck",
                            "duration": 2852,
                            "album_art_uri": ("https://cdn.example.test/sysk.png?version=1"),
                        }
                    },
                )()
            },
    ...
    controller._tunein_media_url = lambda service, item: "https://podcast.example.test/episode.mp3"
    snapshot = controller.refresh()
    ...
    metadata = safe_xml_fromstring(living.played_uri["meta"])
    assert metadata.findtext(".//{http://purl.org/dc/elements/1.1/}title") == ("The Manhattan Grid")
    assert (
        metadata.findtext(".//{urn:schemas-upnp-org:metadata-1-0/upnp/}albumArtURI")
        == "https://cdn.example.test/sysk.png?version=1"
    )
    assert (
        metadata.findtext(".//{urn:schemas-upnp-org:metadata-1-0/upnp/}album")
        == "Stuff You Should Know"
    )
    assert metadata.findtext(".//{http://purl.org/dc/elements/1.1/}creator") == ("Josh and Chuck")
        ...
        "households": [
            {
                "id": "HH1",
                "rooms": [{"uid": "R1"}, {"uid": "R2"}],
                "groups": [{"coordinatorUid": "R1", "memberUids": ["R1", "R2"]}],
            }
        ],
    }
    controller.refresh = lambda **kwargs: snapshot  # type: ignore[method-assign]
...
def test_cached_hosts_are_limited_to_private_lan_addresses():
    living = FakeZone("R1", "Living Room", "10.0.0.2")
    living._visible_zones = {living}
    state = PersistentState(
        selected_room_uid="",
        cached_hosts=["8.8.8.8", "10.0.0.2"],
    )
    state.save = lambda path=None: None  # type: ignore[method-assign]
    factory_calls = []
    controller = SonosController(
        discover_fn=lambda **kwargs: set(),
        soco_factory=lambda host: factory_calls.append(host) or living,
        network_scan_fn=lambda **kwargs: set(),
        persistent_state=state,
    )

    assert controller._discover_zones() == [living]
    assert factory_calls == ["10.0.0.2"]
    assert state.cached_hosts == ["10.0.0.2"]


def test_artwork_url_blocks_internal_remote_targets():
    assert SonosController._safe_artwork_url("/getaa?id=1", "10.0.0.2") == (
        "http://10.0.0.2:1400/getaa?id=1"
    )
    assert SonosController._safe_artwork_url("http://10.0.0.2:1400/getaa?id=1", "10.0.0.2")
    assert (
        SonosController._safe_artwork_url("https://cdn.radiotime.com/cover.jpg", "10.0.0.2")
        == "https://cdn.radiotime.com/cover.jpg"
    )
    assert (
        SonosController._safe_artwork_url("https://images.example.test/cover.jpg", "10.0.0.2") == ""
    )
    assert (
        SonosController._safe_artwork_url("https://cdn.radiotime.com:444/cover.jpg", "10.0.0.2")
        == ""
    )
    assert SonosController._safe_artwork_url("https://8.8.8.8/cover.jpg", "10.0.0.2") == ""
    assert (
        SonosController._safe_artwork_url("http://10.0.0.2:not-a-port/cover.jpg", "10.0.0.2") == ""
    )
    assert (
        SonosController._safe_artwork_url("http://169.254.169.254/latest/meta-data", "10.0.0.2")
        == ""
    )


def test_artwork_url_allows_only_the_reviewed_mytuner_station_host():
    station_art = "https://static.mytuner-radio.net/media/tvos_radios/example.png"
    assert SonosController._safe_artwork_url(station_art, "10.0.0.2") == station_art
    assert (
        SonosController._safe_artwork_url(
            "https://evil.static.mytuner-radio.net/example.png",
            "10.0.0.2",
        )
        == ""
    )
    assert SonosController._safe_artwork_url("https://127.0.0.1/secret", "10.0.0.2") == ""


def test_tunein_playlist_fetch_rejects_untrusted_hosts():
    class FakeTuneIn:
        @staticmethod
        def get_media_uri(_episode_id):
            return "http://169.254.169.254/latest/meta-data"

    class Episode:
        id = "episode-1"

    with pytest.raises(ControllerError, match="invalid podcast media URL"):
        SonosController._tunein_media_url(FakeTuneIn(), Episode())


def test_select_room_remembers_exact_member_without_regrouping():
    state = PersistentState()
    state.save = lambda path=None: None  # type: ignore[method-assign]
    controller = SonosController(persistent_state=state)
    controller.refresh = lambda **kwargs: {  # type: ignore[method-assign]
        "households": [
            {
                "rooms": [
                    {"uid": "R1"},
    ...
    controller.select_room("R2")
    assert state.selected_room_uid == "R2"
    with pytest.raises(ControllerError, match="Unknown Sonos room"):
        controller.select_room("missing")


def test_group_model_omits_hidden_bonded_components():
    living = FakeZone("R1", "Living Room", "10.0.0.2")
    hidden_sub = FakeZone("SUB1", "Living Room Sub", "10.0.0.3")
    group = FakeGroup(living, [living, hidden_sub])
    controller = SonosController(persistent_state=PersistentState())

    model = controller._group_model(group, {"R1"})

    assert model["memberUids"] == ["R1"]
    assert model["label"] == "Living Room"
